...
setInterval(() => {
  ...
  for (const room of ROOMS.values()) {
    ...
    const me = room.match.getPlayer(socket_of_interest);
    if (me && me.firing) console.log(`[INSTR tick] phase=${room.match.phase} p.firing=${me.firing} ammo=${me.ammo} t=${room.match._now().toFixed(2)}`);
...
io.on("connection", (socket) => {
  ...
  socket.on("game:input", (data) => {
    ...
    if (data && typeof data.firing === "boolean" && data.firing) {
      const p = room.match.getPlayer(socket.id);
      if (p) console.log(`[INSTR input] firing=true phase=${room.match.phase} p.firing=${p.firing} ammo=${p.ammo} t=${room.match._now().toFixed(2)}`);
    }
  });
